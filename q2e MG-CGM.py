...
Ex0 = np.pi*np.cos(np.pi*xx)*np.sin(np.pi*yy)/epsilon0
Ey0 = np.pi*np.sin(np.pi*xx)*np.cos(np.pi*yy)/epsilon0
Exy0 = -np.stack((Ey0, Ex0), axis=2)[1:-1, 1:-1, :]

rms= np.linalg.norm(np.reshape(phi-phi0,(phi.shape[0]*phi.shape[1],1)))
print(rms)
'''
######## 电场的可视化 ########
...
def plotElectricPotential():
    ...
    绘制电势大小
    '''
    plt.pcolor(x, y, phi, cmap='jet',norm=LogNorm())
    plt.colorbar()
    plt.xlabel('X')
    plt.ylabel('Y')
...
def plotEquipotentialSurface():
    ...
    绘制等势线
    '''
    plt.contour(x, y, np.log10(phi), 10, colors='k',
...
def plotElectricFieldIntensity():
    ...
    plt.pcolor(x[1:-1], y[1:-1], E, cmap='jet',norm=LogNorm(vmin=1e-5))
    plt.colorbar()
    plt.xlabel('X')
    plt.ylabel('Y')
...
def writecsv(data):  # 导出线条
    ...
    dataframe.to_csv("error-MG-CGM.csv",
